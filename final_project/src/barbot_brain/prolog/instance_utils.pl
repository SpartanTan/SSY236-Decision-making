...
getClassPath(Class, Class_path):-
	...
	( % Class has already a URI
	  ...
	  % When the class does not have URL, will get the knowrob path
        ( TempClass='http://www.semanticweb.org/vamsi/ontologies/2023/0/group2-ontology-3#',
...
getIngredients(Individual, I):-
	getClassPath(Individual, Individual_path),
	owl_has(Individual_path, drinkOntology:'hasingredients', I).

getIngredientIDs(Individual, ID):-
	getIngredients(Individual, Ingredients),
	owl_has(Ingredients, drinkOntology:'hasID', ID).